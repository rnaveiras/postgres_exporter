...
SELECT V.pid::text
  ...
  JOIN pg_stat_all_tables AS T ON (T.relid = V.relid) /*postgres_exporter*/`
)
...
func NewStatVacuumProgressScraper() Scraper {
	return &statVacuumProgressScraper{
		running: prometheus.NewDesc(
			...
			"VACUUM is running",
			[]string{"pid", "query_start", "datname", "relname"},
			nil,
		),
...
func (c *statVacuumProgressScraper) Scrape(ctx context.Context, conn *pgx.Conn, version Version, ch chan<- prometheus.Metric) error {
	...
	var pid, queryStart, datname, relname, phase string
	...
	for rows.Next() {
		...
		// postgres_stat_vacuum_progress_running
		ch <- prometheus.MustNewConstMetric(c.running, prometheus.GaugeValue, 1, pid, queryStart, datname, relname)

		switch phase {
		case "initializing":
			// postgres_stat_vacuum_progress_phase_initializing
			ch <- prometheus.MustNewConstMetric(c.phaseInitializing, prometheus.GaugeValue, 1, pid, queryStart, datname, relname)
		case "scanning heap":
			// postgres_stat_vacuum_progress_phase_scanning_heap
			ch <- prometheus.MustNewConstMetric(c.phaseScanningHeap, prometheus.GaugeValue, 1, pid, queryStart, datname, relname)
		case "vacuuming indexes":
			// postgres_stat_vacuum_progress_phase_vacuuming_indexes
			ch <- prometheus.MustNewConstMetric(c.phaseVacuumingIndexes, prometheus.GaugeValue, 1, pid, queryStart, datname, relname)
		case "vacuuming heap":
			// postgres_stat_vacuum_progress_phase_vacuuming_heap
			ch <- prometheus.MustNewConstMetric(c.phaseVacuumingHeap, prometheus.GaugeValue, 1, pid, queryStart, datname, relname)
		case "cleaning up indexes":
			// postgres_stat_vacuum_progress_phase_cleaning_up_indexes
			ch <- prometheus.MustNewConstMetric(c.phaseCleaningUpIndexes, prometheus.GaugeValue, 1, pid, queryStart, datname, relname)
		case "truncating heap":
			// postgres_stat_vacuum_progress_phase_truncating_heap
			ch <- prometheus.MustNewConstMetric(c.phaseTruncatingHeap, prometheus.GaugeValue, 1, pid, queryStart, datname, relname)
		case "performing final cleanup":
			// postgres_stat_vacuum_progress_phase_performing_final_cleanup
			ch <- prometheus.MustNewConstMetric(c.phasePerformingFinalCleanup, prometheus.GaugeValue, 1, pid, queryStart, datname, relname)
		}

		// postgres_stat_vacuum_progress_heap_blks_total
		ch <- prometheus.MustNewConstMetric(c.heapBlksTotal, prometheus.GaugeValue, heapBlksTotal, pid, queryStart, datname, relname)

		// postgres_stat_vacuum_progress_heap_blks_scanned
		ch <- prometheus.MustNewConstMetric(c.heapBlksScanned, prometheus.GaugeValue, heapBlksScanned, pid, queryStart, datname, relname)

		// postgres_stat_vacuum_progress_heap_blks_vacuumed
		ch <- prometheus.MustNewConstMetric(c.heapBlksVacuumed, prometheus.GaugeValue, heapBlksVacuumed, pid, queryStart, datname, relname)

		// postgres_stat_vacuum_progress_index_vacuum_count
		ch <- prometheus.MustNewConstMetric(c.indexVacuumCount, prometheus.GaugeValue, indexVacuumCount, pid, queryStart, datname, relname)

		// postgres_stat_vacuum_progress_max_dead_tuples
		ch <- prometheus.MustNewConstMetric(c.maxDeadTuples, prometheus.GaugeValue, maxDeadTuples, pid, queryStart, datname, relname)

		// postgres_stat_vacuum_progress_num_dead_tuples
		ch <- prometheus.MustNewConstMetric(c.numDeadTuples, prometheus.GaugeValue, numDeadTuples, pid, queryStart, datname, relname)
